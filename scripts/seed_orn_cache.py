...
1. PreDB APIs (recent releases)
2. NZBHydra2 (if configured)
3. CSV import (manual mappings)
4. Newznab indexers
...
class ORNSeeder:
    ...
    async def seed_from_predb(self, limit: int = 1000) -> int:
        ...
        """
        logger.info(f"Seeding ORN cache from PreDB APIs (limit: {limit})")
                ...
                try:
                    ...
                    async with self.session.get(url, params=params) as response:
                        if response.status != 200:
                            logger.warning(
                                f"{api['name']} returned status {response.status}"
                            )
                            continue
                        ...
                        # Parse response
                        releases = []
                        if api["name"] == "predb.ovh":
                            if data.get("status") == "success":
                                releases = data.get("data", [])
                        elif api["name"] == "predb.me":
                            if data.get("status") == "success":
                                releases = data.get("data", [])
                        ...
                        for release in releases:
                            try:
                                ...
                                # Generate hash candidates from release name
                                hashes = self._generate_hash_candidates(name)

                                for hash_candidate in hashes:
                                    await self._add_mapping(
                                        db, hash_candidate, name, f"predb_{api['name']}"
                                    )
