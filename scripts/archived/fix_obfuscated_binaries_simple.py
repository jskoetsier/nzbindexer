#!/usr/bin/env python
"""
Script to fix detection of obfuscated binary posts - simplified version
"""
...
async def update_article_processing_code():
    ...
    # Create the new content with the updated _process_binary_post method
    new_content = """
    ...
    # Replace the existing _process_binary_post method with the new one
    pattern = r"async def _process_binary_post\([^)]*\):[^}]*?\n    \"\"\"\n    Process a binary post from a newsgroup\n    \"\"\"\n.*?# Update total parts if we have a new value.*?binaries\[binary_key\]\[\"total_parts\"\] = total_parts"

    if re.search(pattern, content, re.DOTALL):
        updated_content = re.sub(pattern, new_content.strip(), content, flags=re.DOTALL)
            ...
            try:
                ...
                try:
                    ...
                    if yenc_begin and yenc_name:
                        ...
                        if 'part_match' in locals() and part_match:
